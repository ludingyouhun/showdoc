...
class FromCommentsController extends BaseController {
    ...
    private function clearContent($content){
        ...
        $param = array();
        foreach ($content["param"] as $paramInfo){
            
            $data = array();
            $data["name"] = $paramInfo[0];
            $data["type"] = $paramInfo[2];
            $data["value"] = "";
            if($paramInfo[1] == "必填"){
                $data["require"] = 1;
            }else{
            ...
            $data["remark"] = $paramInfo[3];
            array_push($param,$data);
        }
        $result["request"]["params"]["urlencoded"] = $param;
        $header = array();
        $headerInfo["name"] = "User-Token";
        $headerInfo["type"] = "User-Token";
        $headerInfo["value"] = "";
        $headerInfo["require"] = "User-Token";
        $headerInfo["remark"] = "用户授权token";
        array_push($header,$headerInfo);
        $result["request"]["headers"] = $header;
        
        $result["response"]["responseText"] = "";
        $result["response"]["responseOriginal"] = "";
        $result["response"]["responseExample"] = $content["return"];
        $returnParam = array();
        foreach ($content["return_param"] as $return){
            $data = array();
            $data["name"] = $return[0];
            if($return[1] == "[]"){
                $data["type"] = "object";
            ...
            $data["remark"] = $return[2];
            
            array_push($returnParam,$data);
        }
        $result["response"]["responseParamsDesc"] = $returnParam;
        $result["info"] = $info;
        return $result;
    }
    //解析content，返回数组
